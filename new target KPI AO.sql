...
SET	SetUp_Plans_Target		= (SELECT SUM(tar_SetUp) 
	...
	,  SetUp_BP_Rate		= CASE WHEN SetUp_BP IS NOT NULL THEN ( SetUp_BP / SetUp_BP_Denom ) END
	,  SetUp_FP_Rate		= CASE WHEN SetUp_BP IS NOT NULL THEN ( SetUp_FP / SetUp_BP_Denom ) END
	,  Settle_BP_Rate		= CASE WHEN Settle_BP IS NOT NULL THEN ( Settle_BP / Settle_BP_Denom ) END
	,  Settle_FP_Rate		= CASE WHEN Settle_FP IS NOT NULL THEN ( Settle_FP / Settle_BP_Denom ) END
...
UPDATE #KPIs
SET	
	SetUp_Plans_Target = CEILING(( CONVERT(DECIMAL,SetUp_Plans_Target) /
	...
	* (SELECT TargetValue FROM risk.dbo.Calendar oCalendar
	...
	,  Settle_Plans_Target = CEILING(( CONVERT(DECIMAL,Settle_Plans_Target) /
	 (SELECT SUM(TargetValue) FROM risk.dbo.Calendar oCalendar
	WHERE oCalendar.W = kpi.W
	AND	oCalendar.Y = kpi.Y
	AND	oCalendar.IsHoliday = 0 ) )
