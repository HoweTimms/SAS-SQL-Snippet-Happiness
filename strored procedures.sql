...
BEGIN
/*get row values*/
SELECT @BookOn_Month = BookOn_Month, @BookOn_Year = BookOn_Year
FROM @tbl
WHERE RowID = @iRow
/*perform operations with single row*/
PRINT 'My cursor row  |  ' + CONVERT(varchar,@iRow) + '  |  ' + @BookOn_Month + '  |  ' + @BookOn_Year
SET  @ExecString = @BookOn_Month + @BookOn_Year
EXEC sp_AssetLocation @ExecString 
/*go to next row*/
	SET @iRow = @iRow + 1
...
CREATE PROC sp_AssetLocation @DatePart VARCHAR(5)
AS
/* Local Variables */
